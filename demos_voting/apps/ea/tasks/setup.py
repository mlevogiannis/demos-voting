...
from demos_voting.apps.ea.tasks import cryptotools, pdf
from demos_voting.apps.ea.tasks.masks import apply_mask
...
from demos_voting.common.utils.permutation import permute
from demos_voting.common.utils.setup import insert_into_db
...
def election_setup(election_obj, language):
    ...
    progress = {'current': 0, 'total': election.ballots_cnt * 2}
    ...
    async_result2 = thread_pool.apply_async(_gen_ballot_crypto,
        (q_list, min(conf.BATCH_SIZE, election.ballots_cnt)))
    ...
    for lo in range(100, election.ballots_cnt + 100, conf.BATCH_SIZE):
        ...
        if hi - 100 < election.ballots_cnt:
            async_result2 = thread_pool.apply_async(_gen_ballot_crypto,
                (q_list, min(conf.BATCH_SIZE, election.ballots_cnt + 100 - hi)))
            ...
            for p_tag, crypto_qo_list in zip(['A', 'B'], crypto_sqo_list):
                ...
                for q_index, crypto_o_list in enumerate(crypto_qo_list):
                    ...
                    for votecode, (com, decom, zk1, zk_state) \
                        ...
                        optionv_id = (q_index * election.max_options_cnt) + votecode
