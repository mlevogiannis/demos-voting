...
from demos_voting.apps.abb.models import Election, Question, Option, Ballot, Part, PQuestion, POption
...
class ElectionSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def get_certificate_url(self, obj):
        if obj.certificate:
            return reverse('abb-media:certificate', kwargs={'election_id': obj.id}, request=self.context['request'])
    ...
    class Meta:
        ...
        extra_kwargs = {
            'url': {
                'view_name': 'abb-api:election-detail',
                'lookup_field': 'id',
            },
        }
...
class BallotSerializer(nested_serializers.NestedHyperlinkedModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            'url': {
                'view_name': 'abb-api:ballot-detail',
