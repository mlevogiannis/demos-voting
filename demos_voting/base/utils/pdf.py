...
from demos_voting.base.models import Election, Question
...
def _load_image(filename, width=None, height=None):
    ...
    p = os.path.join(settings.BASE_DIR, 'demos_voting/base/static/base/images', filename)
    with open(p, 'rb') as f:
        b = io.BytesIO(f.read())
...
def generate(*ballots):
    ...
    bulletin_board_text = _("Bulletin Board")
    vote_collector_text = _("Vote Collector")
    ...
    for ballot in ballots:
        ...
        for part in ballot.parts.all():
            ...
            vote_collector_url = urljoin(settings.DEMOS_VOTING_URLS['vote_collector'], quote("%s/%d/%s/%s/" %
                (election.id, ballot.serial_number, part.tag, ballot.credential)))

            bulletin_board_url = urljoin(settings.DEMOS_VOTING_URLS['bulletin_board'], quote("%s/" % election.id))

            qr = qrcode.QRCode(error_correction=qrcode.constants.ERROR_CORRECT_M)
            qr.add_data(vote_collector_url)
            qr.make(fit=True)
            qr_data = io.BytesIO()
            ...
            page_footer_table = Table(
                data = [
                    [_kv_paragraph(vote_collector_text, vote_collector_url, mode='u')],
                    [_kv_paragraph(bulletin_board_text, bulletin_board_url, mode='u')],
                    ["", "", part.tag],
                    [qr_img, logo_img, ""],
