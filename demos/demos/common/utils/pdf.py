...
from demos.common.models import Election
...
def generate(*ballots):
    ...
    if election.votecode_type_is_short:
        ...
        if election.type_is_referendum:
            max_options = settings.DEMOS_MAX_REFERENDUM_OPTIONS
        elif election.type_is_election:
    ...
    for ballot in ballots:
        ...
        for part in ballot.parts.all():
            ...
            abb_url = urljoin(settings.DEMOS_URLS['abb'], quote("%s/" % election.id))
            vbb_url = urljoin(settings.DEMOS_URLS['vbb'], quote("%s/%s/" % (election.id, part.token)))
