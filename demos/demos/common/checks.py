...
from demos.common.models import PrivateApiUser
from demos.common.utils.private_api import PrivateApiAuth
...
def private_api_check(app_configs, **kwargs):
    ...
    for local_app in settings.DEMOS_APPS:
        for remote_app in PrivateApiUser.APP_DEPENDENCIES[local_app]:
            
            try:
                r = requests.get(
                    url=urljoin(settings.DEMOS_API_URLS[remote_app], 'api/_private/test/'),
                    verify=getattr(settings, 'DEMOS_PRIVATE_API_VERIFY_SSL', True),
...
def file_storage_check(app_configs, **kwargs):
    ...
    try:
        with tempfile.TemporaryFile(dir=settings.DEMOS_DATA_DIR):
            pass
    except Exception as e:
        messages.append(
...
def security_check(app_configs, **kwargs):
    ...
    if settings.DEMOS_APPS > 1:
        messages.append(
            checks.Warning("DEMOS Apps must be isolated in order to support voter privacy.",
                           hint="Install each one of (ea, bds, abb, vbb) to different servers.",
                           id='common.W001')
        )
    
    urls = settings.DEMOS_URLS.values() + settings.DEMOS_PRIVATE_API_URLS.values()
    
    if not all(url.startswith('https://') for url in urls):
        messages.append(
            checks.Warning("One or more of the configured DEMOS URLs do not use the HTTPS protocol.",
                           id='common.W002')
        )
